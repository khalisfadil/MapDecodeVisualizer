...
/**
 * @brief Constructs the callbackStaticVoxelColor object.
 * Initializes the receivedXYZ_ buffer with NaN values to mark unused slots.
 */
CallbackStaticVoxelColor::CallbackStaticVoxelColor()
    : receivedXYZ_(MAX_NUM_POINT_STATIC, Eigen::Matrix<uint32_t, 3, 1>::Zero())
{}
 ...
 */
void CallbackStaticVoxelColor::process(const std::vector<uint8_t>& data, Voxel& voxel) {
